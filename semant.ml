...
let check stmts vars funcs =
	...
	let rec check_func_locals var_map func_map = function
	  | [] -> []
	  | Bind(t, s) as st :: tail -> 
	    let (_, _, (t1, e1)) = check_stmt var_map func_map st in
	    (t1, e1) :: check_func_locals var_map func_map tail
	...
	let rec check_func_body var_map func_map = function
	  | [] -> []
	...
	let rec check_expr var_map func_map = function 
	  ...
	  | Id var -> (var_map, func_map, (type_of_id var_map var, SId var))
	  ...
	  | Access(var, ex) -> (* TODO: ensure var is of list type and ex results in an int *)
	    ...
	    if t1 = Lst && t2 = Int then (var_map, func_map, SAccess(var, (t2, e2)))
	  ...
	  | Slice(var, ex1, ex2) -> (* ensure var is of list type and ex1 and ex2 result in int *)
	    ...
	    if t1 = Lst && t2 = Int && t3 = Int then (var_map, func_map, SSlice(var, (t2, e2), (t3, e3)))
		else raise (Failure ("list slice indices must be of type int"))
	in
	let rec check_stmt var_map func_map = function

	  | Expr ex -> check_expr var_map func_map ex

	  | Bind(ty, st) ->
	    let m = add_var var_map st ty in
	    (m, func_map, (ty, SBind(ty, st)))
	 
	  | FuncDef(vdec, st1_lst, st2_lst) -> (* TODO: add func def to map *)
	  	let Bind(ty, name) = vdec
	  	and params = check_stmt_list StringMap.empty func_map st1_lst
	  	and locals = check_func_locals var_map st1_lst
	  	and body = check_func_body var_map st2_lst
	 	in
	 	(var_map, func_map, SFuncDef({ srtyp=ty; sfname=name; sformals=params; slocals=locals; sbody=body }))

	  | If(ex, st1_lst, st2_lst) -> 
	    (var_map, func_map, SIf(check_bool_expr ex, check_stmt_list var_map func_map st1_lst, check_stmt_list var_map func_map st2_lst))
	  	
	  | Elif(ex, st_lst) -> (var_map, func_map, SElif(check_bool_expr ex, check_stmt_list var_map func_map st_lst))
	  	
	  | Else st_lst -> (var_map, func_map, SElse(check_stmt_list var_map func_map st_lst))
	  	
	  | While(ex, st_lst) -> (var_map, func_map, SWhile(check_bool_expr ex, check_stmt_list var_map func_map st_lst))
	  ...
	  | Range(st1, ex, st2_lst) ->
	  	let (m1, _, (t1, e1)) = check_stmt var_map func_map st1 
	  	and (m2, _, (t2, e2)) = check_expr m1 func_map ex 
	    and sst_lst = check_stmt_list m func_map st2_lst in
	  	if t1 = t2 then
	  	  match t1 with
	  	    | _ when (t1 = Int && t2 = Int) -> (m2, func_map, SRange((t1, e1), (t2, e2), sst_lst))
	  ...
	  | Do(st_lst, ex) -> (var_map, func_map, SDo(check_stmt_list var_map func_map st_lst, check_bool_expr ex))
	  ...
	  | Assign(ex1, ex2) -> 
	  	...
	  	in
	  	if t1 = t2 then (m2, func_map, (t1, SAssign((t1, e1), (t2, e2))))
	  	else raise (Failure err)

	  | DecAssign(st, ex) -> 
	    let (m1, _, (t1, e1)) = check_stmt var_map func_map st 
	    and (m2, _, (t2, e2)) = check_expr m1 func_map ex in
	    let err = "illegal assignment, expected expression of type " ^ t1 ^ " but got expression of type " ^ t2
		in
		if t1 = t2 then (m2, func_map, (t1, SDecAssign(e1, (t2, e2))))
		else raise (Failure err)
	...
	in let rec check_stmt_list var_map func_map = function 
	  | [] -> []
	  | s :: sl -> 
